fun main() {
    ...
    data class Engine(
        ...
        fun sumParts(input: String) = symbols.flatMap {
            val extracted = extract(it, input)
            extracted
        }.sum()

        fun sumGearRatios(input: String) = symbols.sumOf {
            val extracted = extract(it, input)
            if (extracted.size == 2) {
                extracted.fold(1, Int::times)
            } else {
                0
            }
        }

        private fun extract(it: Int, input: String): MutableList<Int> {
            val left = it - 1
            val right = it + 1
            val up = it - len
            val down = it + len
            ...
            if (contains(left)) {
                extracted.add(extractNumberLeft(input, left).toInt())
            }
            if (contains(right)) {
                extracted.add(extractNumberRight(input, right).toInt())
            }
            return extracted
        }
    ...
    fun part2(input: List<String>) = input.foldIndexed(Engine(input.first().length)) { lineIdx, acc, current ->
        val globalIdx = lineIdx * acc.len
        current.foldIndexed(acc) { idx, acc2, symbol ->
            val globalLineIdx = globalIdx + idx
            if (symbol.isDigit()) {
    ...
    }.sumGearRatios(input.joinToString(""))
